...
#include "planarRefinementIterator.H"
#include "polyMesh.H"
#include "fvMesh.H"
#include "Time.H"
...
namespace Foam
{
defineTypeNameAndDebug(planarRefinementIterator, 0);
}
...
Foam::planarRefinementIterator::planarRefinementIterator
(
    polyMesh& mesh,
...
Foam::Map<Foam::label> Foam::planarRefinementIterator::setRefinement
(
...
)
{
    ...
    do
    {
        ...
        fvMesh& fvMeshRef = refCast<fvMesh>(mesh_);
        fvMeshRef.preChange();
        ...
        if (debug)
        {
            Pout<< "planarRefinementIterator : refining "
        ...
        if (nCuts == 0)
        {
            if (debug)
            {
                Pout<< "planarRefinementIterator : exiting iteration since no valid"
        ...
        // Dynamic-solver addition: map registered finite-volume fields and
        // notify mesh-aware objects after each directional split.
        fvMeshRef.topoChange(map());
