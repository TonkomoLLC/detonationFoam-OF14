...
namespace Foam
{
    defineTypeNameAndDebug(planarMultiDirRefinement, 0);
}


// * * * * * * * * * * * * * Private Statc Functions * * * * * * * * * * * * //

void Foam::planarMultiDirRefinement::addCells
(
...
void Foam::planarMultiDirRefinement::update
(
    const Map<label>& splitMap,
    vectorField& field
)
...
Foam::labelList Foam::planarMultiDirRefinement::splitOffHex(const primitiveMesh& mesh)
{
...
void Foam::planarMultiDirRefinement::refineHex8
(
    polyMesh& mesh,
...
)
{
    if (debug)
    {
        Pout<< "planarMultiDirRefinement : Refining hexes " << hexCells.size()
            << endl;
    }
    ...
    if (debug)
    {
        Pout<< "planarMultiDirRefinement : updated mesh at time "
            << mesh.time().name() << endl;
    }
...
void Foam::planarMultiDirRefinement::refineAllDirs
(
    polyMesh& mesh,
...
{
    // Iterator
    planarRefinementIterator refiner(mesh, cutter, cellWalker, writeMesh);

    forAll(cellDirections, dirI)
    {
        if (debug)
        {
            Pout<< "planarMultiDirRefinement : Refining " << cellLabels_.size()
        ...
        if (dirField.size() == 1)
        {
            ...
            if (debug)
            {
                Pout<< "planarMultiDirRefinement : Uniform refinement:"
                    << dirField[0] << endl;
            }
        ...
        if (debug)
        {
            Pout<< "planarMultiDirRefinement : Done refining direction " << dirI
...
void Foam::planarMultiDirRefinement::refineFromDict
(
    polyMesh& mesh,
    List<vectorField>& cellDirections,
